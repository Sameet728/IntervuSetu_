...
function BadgeChip({ badge }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-1 rounded-full border ${BADGE_STYLES[badge] || 'bg-zinc-100 border-zinc-200 text-zinc-700'}`}>
      <span className="text-[12px]">{icons[badge]}</span> {badge}
    </span>
  )
}
...
function ScoreBar({ val, max = 10, color = 'bg-blue-500' }) {
  return (
    <div className="w-full bg-zinc-100 dark:bg-zinc-800 rounded-full h-1.5 overflow-hidden">
      <div className={`${color} h-1.5 rounded-full transition-all duration-500 ease-out`} style={{ width: `${Math.min((val / max) * 100, 100)}%` }} />
    </div>
  )
}
      ...
      <nav className="border-b border-zinc-200 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0 border-l border-zinc-200 dark:border-zinc-800 pl-4">
            <p className="font-semibold text-zinc-900 dark:text-zinc-100 text-sm truncate" style={{ fontFamily: 'Poppins, sans-serif' }}>{template?.title || 'Leaderboard'}</p>
            <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider mt-0.5">{template?.role} • {template?.interviewType}</p>
          </div>
        ...
        {tab === 'analytics' && analytics && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Total Candidates', val: analytics.totalAttempts, icon: <Users className="w-5 h-5 text-blue-600 dark:text-blue-500" /> },
                { label: 'Completed', val: analytics.completedAttempts, icon: <CheckCircle2 className="w-5 h-5 text-green-600 dark:text-green-500" /> },
                { label: 'Avg Score', val: analytics.avgScore ? `${analytics.avgScore}/10` : '—', icon: <Star className="w-5 h-5 text-amber-500" /> },
                { label: 'Completion Rate', val: `${analytics.completionRate}%`, icon: <TrendingUp className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> },
              ].map((s, i) => (
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                <p className="text-xs font-bold text-zinc-400 mb-5 tracking-wider uppercase">STATUS DISTRIBUTION</p>
                {Object.entries(analytics.statusDistribution || {}).map(([k, v]) => (
                  <div key={k} className="flex items-center justify-between mb-3 last:mb-0">
                    <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300 capitalize">{k}</span>
                    <div className="flex items-center gap-3">
                      <div className="w-24 bg-zinc-100 dark:bg-zinc-800 rounded-full h-2">
                        <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${analytics.totalAttempts > 0 ? (v / analytics.totalAttempts) * 100 : 0}%` }} />
                      </div>
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                <p className="text-xs font-bold text-zinc-400 mb-5 tracking-wider uppercase">BADGE DISTRIBUTION</p>
                {Object.entries(analytics.badgeDistribution || {}).length === 0
                  ? <p className="text-zinc-500 text-sm italic text-center py-6">No badges assigned yet</p>
                  : <div className="space-y-3">
                      {Object.entries(analytics.badgeDistribution || {}).map(([k, v]) => (
                        <div key={k} className="flex items-center justify-between">
                          <BadgeChip badge={k} />
                          <span className="text-sm font-mono font-bold text-zinc-600 dark:text-zinc-400">{v}</span>
                        </div>
                      ))}
                    </div>
                }
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                <p className="text-xs font-bold text-zinc-400 mb-5 tracking-wider uppercase">SKILL HEATMAP (AVG)</p>
                {analytics.skillHeatmap && Object.entries(analytics.skillHeatmap).map(([k, v]) => (
                  <div key={k} className="flex flex-col mb-4 last:mb-0">
                    <div className="flex justify-between mb-1.5">
                      <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300 capitalize">{k.replace(/([A-Z])/g, ' $1').trim()}</span>
                      <span className="text-xs font-mono font-bold text-zinc-600 dark:text-zinc-400">{v.toFixed(1)}/10</span>
                    </div>
                    <ScoreBar val={v} max={10} color={
                      v >= 8 ? 'bg-green-500' : v >= 6 ? 'bg-amber-500' : 'bg-red-500'
                    } />
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6 bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
              ...
              <input type="number" value={scoreMin} onChange={(e) => setScoreMin(e.target.value)}
                ...
                className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 placeholder-zinc-400 dark:placeholder-zinc-500 font-mono w-28 transition-all" />
              ...
              <div className="relative">
                ...
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
                  className="appearance-none bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg pl-8 pr-8 py-2 text-sm text-zinc-700 dark:text-zinc-300 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 font-medium transition-all">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-zinc-500 text-sm font-medium">Loading leaderboard...</p>
              </div>
            ) : entries.length === 0 ? (
              <div className="text-center py-24 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm">
                ...
                <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2" style={{ fontFamily: 'Poppins, sans-serif' }}>No candidates yet</h3>
                <p className="text-zinc-500 text-sm">No candidates have completed this interview matching your criteria.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {entries.map((entry, idx) => (
                  <motion.div key={entry.interviewId}
                    ...
                    className={`border rounded-xl p-5 cursor-pointer hover:border-blue-400 dark:hover:border-blue-600 hover:shadow-md transition-all duration-200 ${getRankBg(entry.rank)}`}>
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex items-center gap-6 flex-shrink-0 bg-white/50 dark:bg-zinc-950/30 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                        <div className="text-center px-2">
                          <p className={`font-bold text-2xl font-mono leading-none ${
                            entry.overallScore >= 8 ? 'text-green-600 dark:text-green-500' :
                            entry.overallScore >= 6 ? 'text-amber-600 dark:text-amber-500' : 'text-red-600 dark:text-red-500'
                          }`}>{entry.overallScore.toFixed(1)}</p>
                          <p className="text-[10px] font-bold text-zinc-400 mt-1 uppercase tracking-wider">Overall</p>
                        </div>
                        <div className="hidden sm:flex flex-col gap-1.5 w-36 border-l border-zinc-200 dark:border-zinc-700 pl-4">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-semibold text-zinc-500 w-8 uppercase tracking-wide">Tech</span>
                            <ScoreBar val={entry.technicalScore} color="bg-blue-500" />
                            <span className="text-[10px] font-bold text-zinc-700 dark:text-zinc-300 font-mono w-6 text-right">{entry.technicalScore.toFixed(1)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-semibold text-zinc-500 w-8 uppercase tracking-wide">Comm</span>
                            <ScoreBar val={entry.communicationScore} color="bg-indigo-500" />
                            <span className="text-[10px] font-bold text-zinc-700 dark:text-zinc-300 font-mono w-6 text-right">{entry.communicationScore.toFixed(1)}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-semibold text-zinc-500 w-8 uppercase tracking-wide">PS</span>
                            <ScoreBar val={entry.problemSolvingScore} color="bg-amber-500" />
                            <span className="text-[10px] font-bold text-zinc-700 dark:text-zinc-300 font-mono w-6 text-right">{entry.problemSolvingScore.toFixed(1)}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-5 flex-shrink-0 ml-auto">
                        <div className="text-center hidden lg:block bg-zinc-50 dark:bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-100 dark:border-zinc-800">
                          <p className="text-sm font-bold text-zinc-700 dark:text-zinc-300 font-mono flex items-center justify-center gap-1.5"><Clock className="w-3.5 h-3.5 text-zinc-400"/>{fmtTime(entry.totalTimeTaken)}</p>
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          {entry.status === 'Auto-submitted' && (
                            <span className="text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 px-2 py-0.5 rounded-md uppercase tracking-wider flex items-center gap-1"><AlertCircle className="w-3 h-3"/>Auto-submit</span>
                          )}
                        </div>
                    ...
                    {entry.candidateTags?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3 ml-16">
                        {entry.candidateTags.map((tag, ti) => (
                          <span key={ti} className="text-[10px] font-medium text-zinc-600 dark:text-zinc-400 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 px-2.5 py-0.5 rounded-md shadow-sm">{tag}</span>
                        ))}
                      </div>
                    )}
                  </motion.div>
